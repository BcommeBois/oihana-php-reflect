...
> Decoding helpers (`JsonSerializer::decode()`, `CborSerializer::decode()`) are planned. To turn decoded data back into typed objects today, decode with `json_decode($json, true)` / `cbor_decode($bytes)` and pass the array to [`Reflection::hydrate()`](hydration/README.md).
