...
- Bump the `phpunit/phpunit` dev dependency from `^12` to `^13` and update the
  ...
  suite (232 tests) stays green with 100% line coverage. PHPUnit 13 requires
  PHP ≥ 8.4, already the project's minimum.
...
- Traits
  ...
  - JsonSchemaTrait now names a `#[HydrateKey]` property after its primary source key (`keys[0]`) instead of the PHP property name, so the schema describes — and validates — the input shape that `hydrate()` actually consumes (e.g. `#[HydrateKey('user_name')] public string $name` is exposed as the `user_name` property).
- Utils
